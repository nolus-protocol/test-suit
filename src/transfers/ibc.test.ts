...
describe('IBC transfer', () => {
  ...
  test('users should be able to transfer ibc tokens', async () => {
    ...
    const initialUser1Balance = await user1Client.getBalance(
      user1Account.address,
      ibcToken,
    );
    const initialUser2Balance = await user2Client.getBalance(
    ...
    expect(BigInt(nextUser1Balance.amount)).toBe(
      BigInt(initialUser1Balance.amount) - BigInt(transfer.amount),
    );
    expect(BigInt(nextUser2Balance.amount)).toBe(
      BigInt(initialUser2Balance.amount) + BigInt(transfer.amount),
    );
  });
});
