...
describe('Native transfer', () => {
  ...
  test('account should have some balance', async () => {
    ...
    const balance: Coin = await userClient.getBalance(
      userAccount.address,
      NATIVE_TOKEN,
    );
  ...
  test('users should be able to transfer native tokens', async () => {
    ...
    console.log(
      `User1 balance before=(${previousUser1Balance.denom}, ${previousUser1Balance.amount}) after=(${nextUser1Balance.denom}, ${nextUser1Balance.amount})`,
    );
    console.log(
      `User2 balance before=(${previousUser2Balance.denom}, ${previousUser2Balance.amount}) after=(${nextUser2Balance.denom}, ${nextUser2Balance.amount})`,
    );
    ...
    console.log(
      `User3 balance before=(${previousUser3Balance.denom}, ${previousUser3Balance.amount}) after=(${nextUser3Balance.denom}, ${nextUser3Balance.amount})`,
    );
