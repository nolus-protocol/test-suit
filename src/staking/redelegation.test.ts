...
runOrSkip(process.env.TEST_STAKING as string)(
  'Staking Nolus tokens - Redelegation',
  () => {
    ...
    beforeAll(async () => {
      ...
      delegateMsg.value.delegatorAddress =
        redelegateMsg.value.delegatorAddress =
    ...
    test('the successful scenario for tokens redelegation - should work as expected', async () => {
      ...
      const completionTime =
        redelegationEntries.redelegationResponses[0]?.entries[0]
          .redelegationEntry.completionTime.nanos;
    ...
    test('the delegator tries to redelegate tokens to non-existent validator - should produce an error', async () => {
      redelegateMsg.value.validatorDstAddress =
        delegatorWallet.address as string;
