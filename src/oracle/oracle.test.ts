...
import {getUser1Client, getUser1Wallet, getClient, createWallet} from "../util/clients";
...
describe("Oracle contract tests", () => {
    ...
    beforeEach(async () => {
        ...
        // send some tokens
        await userClient.sendTokens(userAccount.address, feederAccount.address, customFees.upload.amount, DEFAULT_FEE);

        let codeId:any = process.env.ORACLE_CODE_ID;
    ...
    test("the feeder should be added" , async () => {
        ...
        const isFeeder = await userClient.queryContractSmart(contractAddress, isFeederMsg);
    ...
    test("feed price should works as expected - one feeder", async () => {
        ...
        await feederClient.execute(feederAccount.address, contractAddress, feedPriceMsg, customFees.exec);
        ...
        const result = await userClient.queryContractSmart(contractAddress, mGOGOL_PriceMsg);
        const result2 = await userClient.queryContractSmart(contractAddress, mAAPL_PriceMsg);
        const result3 = () => userClient.queryContractSmart(contractAddress, failed_PriceMsg);
       ...
       // the price feed period has expired
        ...
        const resultAfter = () => userClient.queryContractSmart(contractAddress, mGOGOL_PriceMsg);
        const result2After = () => userClient.queryContractSmart(contractAddress, mAAPL_PriceMsg);
    ...
    test("feed price should works as expected - even number feeders", async () => {
        ...
        // send some tokens
        await userClient.sendTokens(userAccount.address, feeder2Account.address, customFees.upload.amount, DEFAULT_FEE);
        ...
        const list = await userClient.queryContractSmart(contractAddress, feedersMsg);
        console.log(list);
        ...
        const price = () =>  userClient.queryContractSmart(contractAddress, getPriceMsg);
        ...
        await feeder2Client.execute(feeder2Account.address, contractAddress, feedPrice2Msg, customFees.exec);
        const afterResult = await userClient.queryContractSmart(contractAddress, getPriceMsg);
        ...
        const resultAfterPeriod = () => userClient.queryContractSmart(contractAddress, getPriceMsg);
