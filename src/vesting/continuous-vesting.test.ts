import {createWallet, getClient, getUser1Client, getUser1Wallet} from "../util/clients";
...
describe("Continuous vesting tests", () => {
    ...
    test("created continuous vesting account should works as expected", async () => {
        ...
        assertIsDeliverTxSuccess(await continuousClient.sendTokens(continuousAccount.address, user1Account.address, [HALF_AMOUNT], DEFAULT_FEE));
    });
});
